...
from calc.joint_angles import MOUNT, get_mount, to_anatomical, QuaternionFilter, angles_from_direction
...
# Colours (RGB only — alpha set dynamically each frame)
C_PLANE_SAGITTAL   = (0.20, 0.60, 1.00)   # blue  — sagittal  (flexion / extension)
...
def _plane_mesh(normal: np.ndarray, origin: np.ndarray, size: float, rgb: tuple) -> gl.GLMeshItem:
    """
    Build a translucent square mesh representing an anatomical plane.

    normal : unit vector perpendicular to the plane (in GL display space)
    origin : centre point of the plane square
    size   : half-extent in metres
    rgb    : (r,g,b) 0–1 floats — alpha set dynamically via setColor()
    """
    # Build two orthogonal axes in the plane
    ref = np.array([0., 1., 0.]) if abs(normal[1]) < 0.9 else np.array([1., 0., 0.])
    ax1 = np.cross(normal, ref); ax1 /= np.linalg.norm(ax1)
    ...
    mesh = gl.GLMeshItem(
        meshdata=gl.MeshData(vertexes=corners, faces=faces),
        smooth=False,
        drawEdges=False,
    )
    mesh.setGLOptions('translucent')
    mesh.setColor((*rgb, PLANE_ALPHA_LO))
    return mesh
...
class RenderWidget(QWidget):
    ...
    def _build_scene(self):
        ...
        # ── Anatomical plane meshes (added after side geometry so shoulder pos exists) ──
        self._build_plane_meshes()
    ...
    def _build_plane_meshes(self):
        """
        Create the three anatomical plane quads anchored to the shoulder.
        Normals are expressed in GL display space (after WORLD_ROT).

        Sagittal plane   — normal = GL-X (screen right), separates front/back
        Frontal plane    — normal = GL-Z (into screen),  separates left/right
        Transverse plane — normal = GL-Y (up),           separates up/down
        """
        origin = self._shoulder_pos

        # Anatomical normals → GL display space via WORLD_ROT
        # Sagittal normal  = anatomical Z (RIGHT)
        # Frontal normal   = anatomical X (FORWARD)
        # Transverse normal = anatomical Y (UP)
        sag_normal  = WORLD_ROT.apply(np.array([0., 0., 1.]))   # RIGHT → GL
        fro_normal  = WORLD_ROT.apply(np.array([1., 0., 0.]))   # FWD   → GL
        tra_normal  = WORLD_ROT.apply(np.array([0., 1., 0.]))   # UP    → GL

        self._plane_sagittal   = _plane_mesh(sag_normal, origin, PLANE_SIZE, C_PLANE_SAGITTAL)
        self._plane_frontal    = _plane_mesh(fro_normal, origin, PLANE_SIZE, C_PLANE_FRONTAL)
        self._plane_transverse = _plane_mesh(tra_normal, origin, PLANE_SIZE * 0.7, C_PLANE_TRANSVERSE)

        for p in [self._plane_sagittal, self._plane_frontal, self._plane_transverse]:
            self._view.addItem(p)

    def _update_plane_alphas(self, sagittal_w: float, frontal_w: float):
        """
        Drive plane alpha from the blend weights returned by angles_from_direction.
        sagittal_w + frontal_w == 1.0.
        Dominant plane → PLANE_ALPHA_HI, other → PLANE_ALPHA_LO.
        Transverse plane stays at PLANE_ALPHA_LO always (reference only).
        """
        sag_alpha = PLANE_ALPHA_LO + (PLANE_ALPHA_HI - PLANE_ALPHA_LO) * sagittal_w
        fro_alpha = PLANE_ALPHA_LO + (PLANE_ALPHA_HI - PLANE_ALPHA_LO) * frontal_w

        self._plane_sagittal.setColor(  (*C_PLANE_SAGITTAL,   sag_alpha))
        self._plane_frontal.setColor(   (*C_PLANE_FRONTAL,    fro_alpha))
        self._plane_transverse.setColor((*C_PLANE_TRANSVERSE, PLANE_ALPHA_LO))
    ...
    def _tick_live(self):
        ...
        # ── Update anatomical plane alpha from arm direction ───────────────────
        # Compute weights from the un-scaled anatomical direction (unit vector).
        upper_unit = upper_anat / (np.linalg.norm(upper_anat) + 1e-9)
        _aw = angles_from_direction(upper_unit, upper_unit, Rotation.identity(),
                                    side=side)
        self._update_plane_alphas(_aw["_sagittal_weight"], _aw["_frontal_weight"])

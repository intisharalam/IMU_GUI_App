...
Handles everything BLE-related:
  ...
  - Sending haptic (0x48) and sync (0x53) commands back to a sensor
...
CMD_HAPTIC_REP        = bytes([0x48, 1])   # Strong Click 100%
CMD_HAPTIC_HOLD       = bytes([0x48, 15])  # 750ms Alert 100%
CMD_HAPTIC_SET        = bytes([0x48, 47])  # Buzz 1 100%
CMD_HAPTIC_REST_START = bytes([0x48, 84])  # Ramp Up Medium Smooth
CMD_HAPTIC_REST_END   = bytes([0x48, 88])  # Ramp Up Long Sharp
CMD_HAPTIC_ROM        = bytes([0x48, 52])  # Pulsing Strong 100%
CMD_HAPTIC_DEVIATION  = bytes([0x48, 14])  # Strong Buzz 100%
CMD_HAPTIC_CALIBRATED = bytes([0x48, 10])  # Double Click 100%
CMD_HAPTIC_SESSION    = bytes([0x48, 27])  # Short Double Click Strong
...
class BLEManager:
    ...
    def send_haptic(self, slot_name: str, payload: bytes = CMD_HAPTIC_REP):
        self._schedule(self._send_cmd(slot_name, payload))

    def send_haptic_all(self, payload: bytes = CMD_HAPTIC_REP):
        for name in SLOT_NAMES:
            self.send_haptic(name, payload)

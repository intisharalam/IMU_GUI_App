...
class MainWindow(QMainWindow):
    ...
    def _tick(self):
        ...
        with self._state.lock:
            ...
            current_ex     = self._state.current_exercise
        ...
        if active and calibrated:
            ...
            # Form deviation: arm in wrong plane for the active exercise
            # Flexion exercises expect plane ≥ 45° (sagittal)
            # Abduction exercises expect plane < 45° (frontal)
            if abs(flex) > 20 or abs(abd) > 20:  # only check when arm is raised
                if "FLEX" in current_ex and plane < 30:
                    self._trigger_haptic(now, "Deviation: move forward")
                elif "ABD" in current_ex and plane > 60:
                    self._trigger_haptic(now, "Deviation: move sideways")

            # Compensatory trunk lean during abduction
            if "ABD" in current_ex and trunk_lean > TRUNK_LEAN_LIMIT:

...
# Movement corridor colours
C_CORRIDOR_TUBE  = (0.00, 0.60, 0.25, 0.12)   # translucent green tube (inside = OK)
C_CORRIDOR_EDGE  = (0.00, 1.00, 0.41, 0.50)   # bright green edge line
C_CORRIDOR_WARN  = (1.00, 0.67, 0.00, 0.40)   # amber when arm near edge
CORRIDOR_RADIUS  = 0.055   # tube radius in metres
CORRIDOR_STEPS   = 20      # arc resolution
...
def _corridor_arc_mesh(shoulder: np.ndarray, goal: np.ndarray,
    ...
    """
    Build a tube mesh along the shortest-path arc from shoulder to goal.
    The arc lies in the plane defined by shoulder→DOWN and shoulder→goal,
    which is the correct exercise plane.

    Returns a GLMeshItem forming the corridor tube.
    """
    # Direction from shoulder to goal (normalised)
    ...
    # Start direction = DOWN (I-pose arm direction)
    start_dir = np.array([0., -1., 0.])

    # Slerp from start_dir to goal_dir over n_steps to trace the arc
    # Use Rodrigues rotation formula in the plane spanned by start and goal
    cross = np.cross(start_dir, goal_dir)
    cross_len = np.linalg.norm(cross)
    if cross_len < 1e-6:
        # Parallel — degenerate, just draw a straight line
        ...
        for i in range(n_steps + 1):
            t = i / n_steps
            # Rodrigues rotation of start_dir by angle*t about cross axis
            k = cross / cross_len
            d = (start_dir * np.cos(angle * t) +
                 np.cross(k, start_dir) * np.sin(angle * t) +
                 k * np.dot(k, start_dir) * (1 - np.cos(angle * t)))
            # Scale so the tip reaches goal at t=1
            dist = goal_dist * t
            pts.append(shoulder + d * dist)
        pts = np.array(pts)

    # Build tube around the arc centreline
    verts = []; faces = []
    segs = 8   # circle segments for tube cross-section
    angles = np.linspace(0, 2 * np.pi, segs, endpoint=False)

    for i in range(len(pts)):
        # Local frame at this arc point
        if i < len(pts) - 1:
...
class RenderWidget(QWidget):
    ...
    def __init__(self, state, parent=None):
        ...
        self._goal_pos     = None
        self._corridor_mesh = None   # GLMeshItem or None
    ...
    def set_goal(self, wrist_target_anat: np.ndarray):
        ...
        # Draw movement corridor arc from shoulder to goal
    ...
    def _update_corridor(self, goal_gl: np.ndarray):
        """Rebuild the corridor arc tube toward the goal position."""
        self._clear_corridor()
    ...
    def _toggle_rom_mode(self):
        if not self._rom_mode:
            ...
            self._rom_btn.setStyleSheet(
                "QPushButton{background:#3a3000;color:#ccaa00;border:1px solid #665500;"
                " border-radius:3px;font-size:14px;padding:1px 8px;}"
                "QPushButton:hover{background:#554400;}"
            )
            if self._state:

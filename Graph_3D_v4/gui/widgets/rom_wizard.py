...
def _ensure_rom_table():
    ...
    if "int_rot_deg" not in existing:
        con.execute("ALTER TABLE rom_measurements ADD COLUMN int_rot_deg REAL")
    con.commit()
    con.close()


def _save_rom(flex, abd, ext_rot, int_rot, elbow):
    _ensure_rom_table()
    from datetime import datetime
    con = sqlite3.connect(DB_PATH)
    con.execute(
        "INSERT INTO rom_measurements (date,flex_deg,abd_deg,ext_rot_deg,int_rot_deg,elbow_deg) "
        "VALUES (?,?,?,?,?,?)",
        (datetime.now().isoformat(), flex, abd, ext_rot, int_rot, elbow)
    )
...
def load_last_rom():
    ...
    try:
        ...
        int_rot_col = "int_rot_deg" if "int_rot_deg" in cols else "0.0"
        row = con.execute(
            f"SELECT flex_deg, abd_deg, {ext_rot_col}, {int_rot_col}, elbow_deg "
...
def _svg_int_rotation() -> str:
    return """
<svg viewBox="0 0 160 200" xmlns="http://www.w3.org/2000/svg">
  <!-- Head -->
  <circle cx="80" cy="18" r="12" fill="none" stroke="#008f1a" stroke-width="2"/>
  <!-- Spine -->
  <line x1="80" y1="30" x2="80" y2="100" stroke="#008f1a" stroke-width="2"/>
  <!-- Legs -->
  <line x1="80" y1="100" x2="60" y2="145" stroke="#008f1a" stroke-width="2"/>
  <line x1="80" y1="100" x2="100" y2="145" stroke="#008f1a" stroke-width="2"/>
  <!-- Left arm (inactive) -->
  <line x1="80" y1="45" x2="55" y2="70" stroke="#008f1a" stroke-width="2"/>
  <!-- Right upper arm — horizontal at shoulder, elbow bent down -->
  <line x1="80" y1="45" x2="115" y2="45" stroke="#00ff41" stroke-width="3"/>
  <!-- Right forearm — pointing inward toward belly (rotated inward) -->
  <line x1="115" y1="45" x2="90" y2="72" stroke="#00ff41" stroke-width="3" stroke-dasharray="4,2"/>
  <!-- Elbow joint -->
  <circle cx="115" cy="45" r="4" fill="#00ff41"/>
  <!-- Rotation arc showing inward direction -->
  <path d="M 115 58 A 14 14 0 0 1 95 50" fill="none" stroke="#00ff41" stroke-width="1.5" stroke-dasharray="3,2"/>
  <polygon points="95,50 90,56 100,54" fill="#00ff41"/>
  <!-- Label -->
  <text x="80" y="170" text-anchor="middle" font-size="10" fill="#4a7a4a" font-family="Courier New">ROTATE INWARD</text>
  <text x="80" y="183" text-anchor="middle" font-size="9" fill="#4a7a4a" font-family="Courier New">ELBOW 90°</text>
</svg>"""

STEPS = [
    {
        "title":       "STEP 1 OF 5 — SHOULDER FLEXION",
    ...
    {
        "title":       "STEP 2 OF 5 — SHOULDER ABDUCTION",
    ...
    {
        "title":       "STEP 3 OF 5 — EXTERNAL ROTATION",
    ...
    {
        "title":       "STEP 4 OF 5 — INTERNAL ROTATION",
        "instruction": "Bend elbow to 90°, keep it at your side.\nRotate your forearm INWARD toward your belly. Hold.",
        "angle_key":   "int_rot",
        "state_attr":  "external_rotation",
        "svg_fn":      _svg_int_rotation,
        "unit":        "°",
        "invert":      True,   # internal = negative of external_rotation
        "prompt": "ROTATE FURTHER INWARD",
    },
    {
        "title":       "STEP 5 OF 5 — ELBOW FLEXION",
...
class RomWizard(QDialog):
    ...
    def _finish(self):
        ...
        # Persist to SQLite
        _save_rom(
            self._results.get("flex",    0.0),
            self._results.get("abd",     0.0),
            self._results.get("ext_rot", 0.0),
            self._results.get("int_rot", 0.0),

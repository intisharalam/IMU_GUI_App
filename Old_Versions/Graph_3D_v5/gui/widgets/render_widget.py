...
C_GOAL_FAR   = (0.90, 0.20, 0.20, 0.50)
C_GOAL_NEAR  = (0.95, 0.72, 0.00, 0.60)
C_GOAL_HIT   = (0.05, 0.78, 0.35, 0.75)
...
WORLD_ROT = Rotation.from_euler("Y", -90 + W_ROT, degrees=True)
...
class RenderWidget(QWidget):
    ...
    def set_exercise_guide(self, ex) -> None:
        """Show the ghost-arm guide for the given ExerciseDef."""
    ...
    def _tick_live(self):
        ...
        if self._goal_active and self._goal_pos is not None:
            ...
            if dist < GOAL_HIT_DIST:
                ...
                self._goal_lbl.setStyleSheet("color:#007744; font-size:10px; font-weight:bold;")
                if held >= GOAL_HOLD_S:
